...
const LoanManager = () => {
    ...
    const [activeLoanType, setActiveLoanType] = useState('Regular');  //berts
    ...
    // Fixed archive function with better error handling
    const archiveLoan = async (loan) => {
        try {
            // Check for different possible token names
            ...
            if (!token) {
                // Try to get token from sessionStorage as backup
                token = sessionStorage.getItem('authToken') || 
                       sessionStorage.getItem('token') || 
                       sessionStorage.getItem('access_token') ||
                       sessionStorage.getItem('accessToken');
                       
                console.log('Token from sessionStorage:', token ? 'Yes' : 'No');
            }
            ...
            console.log('Archiving loan:', loan.control_number);
    ...
    useEffect(() => {
        const fetchShareCapital = async () => {
            // Only fetch if form is visible and account exists
            ...
            try {
                ...
                // Clear any share capital related errors
                setErrors(prev => {
                ...
                // Only set error if form is visible (user is actively using the form)
                if (formVisible) {
                    setErrors(prev => ({ 
                        ...prev, 
    ...
    // Fixed fetchLoans function with better error handling
    const fetchLoans = async () => {
        try {
            ...
            // Auto-archive paid-off loans after fetching (with delay to prevent conflicts)
            setTimeout(() => {
            ...
            if (err.response && err.response.status === 401) {
                ...
                // Don't show error popup for fetch failures due to auth
            }
            ...
            // Only show error if it's not a network issue during archive
 ...
 const validateMemberAndLoanTerm = () => {
    ...
    // Check if account holder exists in members list
    if (loanData.account_holder) {
        // Normalize the entered account holder name
        const normalizedEnteredName = loanData.account_holder.toLowerCase().trim();
        
        // Check if the entered account holder matches any member's name
        const memberExists = accountsList.some(member => {
            const fullName = `${member.first_name} ${member.middle_name || ''} ${member.last_name}`
                .toLowerCase()
                .trim()
                .replace(/\s+/g, ' '); // Remove extra spaces
            
            return fullName === normalizedEnteredName;
        });

        if (!memberExists) {
            errors.member = <span><span style={{color: 'orange'}}>Note:</span> Please make sure the name matches exactly as registered</span>;
        }
    }
    ...
    if (loanData.loan_type === 'Regular') {
        if (loanData.loan_period_unit !== 'years') {
            errors.loan_period_unit = <span><span style={{color: 'red'}}>Warning!</span> Regular loans must use years as the term unit </span>;
        }

        const periodInYears = parseInt(loanData.loan_period);
        if (periodInYears < 1 || periodInYears > 4) {
            errors.loan_period = <span><span style={{color: 'red'}}>Warning!</span> Regular loans must have a term between 1 to 4 years </span>;
        }
    }

    if (loanData.loan_type === 'Emergency') {
        if (loanData.loan_period_unit !== 'months') {
            errors.loan_period_unit = <span><span style={{color: 'red'}}>Warning!</span> Emergency loans must use months as the term unit</span>;
        }

        const periodInMonths = parseInt(loanData.loan_period);
        if (periodInMonths < 1 || periodInMonths > 6) {
            errors.loan_period = <span><span style={{color: 'red'}}>Warning!</span> Emergency loans must have a term between 1 to 6 months</span>;
        }
    }
    ...
    const handleLoanSubmit = async (e) => {
        ...
        const validationErrors = {
            ...validateLoanData(),
            ...validateMemberAndLoanTerm()
    };

    if (Object.keys(validationErrors).length > 0) {
        setErrors(validationErrors);
        setShowPopup(true);
        setPopupMessage('Please correct the following errors:');
        return;
    }
            ...
            {formVisible && (
                <form onSubmit={handleLoanSubmit} className="loan-form">
                    ...
                    <div className="form-row">
                        ...
                        {/* Suggestions dropdown (autocomplete style) */}
                                            ...
                                            <tr key={loan.control_number}>
                                                ...
                                                <td style={{ 
                                                    color: loan.status.toLowerCase() === 'ongoing' ? 'red' : 
                                                            loan.status.toLowerCase() === 'paid-off' ? 'green' : 'black' 
                                                }}>
                                                    {loan.status}
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <button
                                                        ...
                                                        disabled={loan.status.toLowerCase() !== 'paid-off'}
                                                        style={{
                                                            backgroundColor: loan.status.toLowerCase() === 'paid-off' ? '#FF9800' : '#ccc',
                                                            color: 'white',
                                                            border: 'none',
                                                            padding: '5px 10px',
                                                            borderRadius: '3px',
                                                            cursor: loan.status.toLowerCase() === 'paid-off' ? 'pointer' : 'not-allowed',
                                                            fontSize: '12px'
                                                        }}
                                                    >
                                                        Archive
                    ...
                    <div className='modal-content'>
                        ...
                        <div className='modal-body'>
                            ...
                            {coMakers.filter(maker => maker && maker.trim() !== '').length === 0 && (
                                <p>No co-makers assigned to this loan.</p>
                            )}
                        </div>
                    </div>
                </div>
